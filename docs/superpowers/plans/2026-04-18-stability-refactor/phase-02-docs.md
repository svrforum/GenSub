...
GenSub 프로젝트에서 및 개발자 공통으로 따르는 규약.
...
## 5. 개발 에이전트 / 에이전트 동작 규약

### 5.1 모델 고정: 

이 프로젝트의 **모든 과 dispatch하는 Agent는 사용**.

- 직접 실행: 세션 진입 시 모델이 인지 확인.
...
코드 컨벤션, 아키텍처 레이어 규칙, 테스트/커밋 규약, 개발 에이전트
에이전트 동작 규칙(모델= 고정 등), 문서 경로 규칙 정리.
EOF
)"
```
